...
def get_time(message):
    ...
    for i in range(len(messages)):
        err_code, messages[i] = myAtoi(messages[i])
        if err_code:
            return err_code, None

    return False, messages

def split_message(update, context):
    ...
    remainder_name = messages[1]
    err_code, remainder_date = get_date(messages[2])
    if err_code:
        print("1")
    ...
    format_time = f"{remainder_time[0]}:{remainder_time[1]} 00"
    json_editor(update.message.chat.id, "name", remainder_name)
    json_editor(update.message.chat.id, "time", format_time)
    json_editor(update.message.chat.id, "date", f"{remainder_date[0]}/{remainder_date[1]}/{remainder_date[2]}")
    json_editor(update.message.chat.id, "id", random.randint(0, 100000))

    context.bot.send_message(chat_id=update.effective_chat.id, text="Remainder added!")
    return False, [remainder_name, remainder_date, remainder_time]

def echo(update, context):
    print("called")
    err_code, message = split_message(update ,context)
    if err_code:
...
def main():
    updater = Updater("5449242501:AAHm23eIiZYt_I0jC_K1eLQ2gH0gY69uxeU", use_context=True)
